const puppeteer = require('puppeteer');
const fs = require('fs');
const path = require('path');

const DATA_DIR = path.join(__dirname, '..', 'puppeteer_data');
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

const SMARTOLT_URL = 'https://joelwifi.smartolt.com';
const EMAIL = 'joelr802@gmail.com';
const PASSWORD = '7rQMHukJVn1R';

function writeResult(data) {
  var str = String(data);
  if (str.length > 100000) str = str.substring(0, 100000) + '\n... [TRUNCATED]';
  fs.writeFileSync(path.join(DATA_DIR, 'result.txt'), str);
}

function writeError(msg) {
  fs.writeFileSync(path.join(DATA_DIR, 'error.txt'), String(msg));
}

(async () => {
  try {
    const browser = await puppeteer.launch({
      headless: false,
      defaultViewport: { width: 1366, height: 768 },
      args: ['--window-size=1366,768']
    });
    const page = await browser.newPage();
    
    // Navegar a SmartOLT
    await page.goto(SMARTOLT_URL, { waitUntil: 'networkidle0', timeout: 30000 });
    console.log('Pagina cargada:', await page.title());
    
    // Si ya está logueado (dashboard), genial
    var title = await page.title();
    if (title !== 'Login') {
      console.log('Ya logueado en:', title);
    } else {
      // Login
      console.log('Haciendo login...');
      
      // Esperar un poco para que cargue el formulario
      await new Promise(function(r) { setTimeout(r, 2000); });
      
      // Intentar diferentes selectores para el email
      var emailInput = await page.$('input[type="email"]') || 
                       await page.$('input[name="email"]') ||
                       await page.$('input[type="text"]') ||
                       await page.$('input.form-control');
      
      var passInput = await page.$('input[type="password"]');
      
      if (emailInput && passInput) {
        console.log('Campos encontrados');
        await emailInput.click({ clickCount: 3 });
        await emailInput.type(EMAIL, { delay: 50 });
        await passInput.click({ clickCount: 3 });
        await passInput.type(PASSWORD, { delay: 50 });
        
        // Buscar botón submit
        var submitBtn = await page.$('button[type="submit"]') ||
                        await page.$('input[type="submit"]') ||
                        await page.$('button.btn-primary');
        
        if (submitBtn) {
          await submitBtn.click();
        } else {
          await page.keyboard.press('Enter');
        }
        
        await page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 20000 }).catch(function(e) {
          console.log('Nav after login:', e.message);
        });
        
        console.log('Post-login title:', await page.title());
      } else {
        console.log('NO se encontraron campos de login');
        // Intentar capturar screenshot para debug
        await page.screenshot({ path: path.join(DATA_DIR, 'login_page.png') });
      }
    }
    
    var finalTitle = await page.title();
    var finalUrl = page.url();
    fs.writeFileSync(path.join(DATA_DIR, 'ready.txt'), 'OK: ' + finalTitle + ' | ' + finalUrl);
    console.log('TITLE:', finalTitle, 'URL:', finalUrl);
    
    // Clonar páginas principales
    var pagesToClone = [
      ['dashboard', '/'],
      ['olt_list', '/olt'],
      ['olt_details', '/olt/olt_details/1/details'],
      ['olt_cards', '/olt/olt_details/1/cards'],
      ['olt_pon_ports', '/olt/olt_details/1/pon_ports'],
      ['olt_uplink_ports', '/olt/olt_details/1/uplink_ports'],
      ['olt_vlans', '/olt/olt_details/1/vlans'],
      ['olt_ip_pools', '/olt/olt_details/1/ip_pools/mgmt'],
      ['configured', '/onu/configured'],
      ['configured_olt1', '/onu/configured?olt_id=1'],
      ['unconfigured', '/onu/unconfigured'],
      ['unconfigured_olt1', '/onu/unconfigured?olt_id=1'],
      ['onu_types', '/onu_types/listing'],
      ['speed_profiles', '/speed_profiles'],
      ['locations', '/locations/listing'],
      ['odbs', '/odbs/listing'],
      ['presets', '/onu_authorization_presets/listing'],
      ['diagnostics', '/diagnostics?signal=critical,warning&olt_id=1'],
      ['graphs', '/graphs'],
      ['system_config', '/system_config'],
      ['general', '/general'],
      ['billing', '/general/listing/billing']
    ];
    
    for (var i = 0; i < pagesToClone.length; i++) {
      var name = pagesToClone[i][0];
      var urlPath = pagesToClone[i][1];
      var fullUrl = SMARTOLT_URL + urlPath;
      
      console.log('[' + (i+1) + '/' + pagesToClone.length + '] ' + name + '...');
      try {
        await page.goto(fullUrl, { waitUntil: 'networkidle0', timeout: 30000 });
        var html = await page.content();
        var size = html.length;
        var outputPath = path.join(DATA_DIR, name + '.html');
        fs.writeFileSync(outputPath, html);
        console.log('  -> ' + name + '.html (' + size + ' bytes) - ' + (await page.title()));
      } catch(e) {
        console.log('  -> ERROR: ' + e.message);
      }
    }
    
    console.log('CLONACION COMPLETA');
    writeResult('CLONING COMPLETE: ' + pagesToClone.length + ' pages');
    
    // Mantener vivo
    setInterval(function() {}, 60000);
  } catch(e) {
    writeError(e.message);
    console.error('FATAL:', e.message);
    console.error(e.stack);
  }
})();
